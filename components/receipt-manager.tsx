...
import { receiptKind, returnDeadline, voluntaryReturnOpen, reversalReasons } from "@/lib/reversals";
import { receiptDownload } from "./checkout-flow";
import { useDialog } from "./use-dialog";
import NumberInput from "./number-input";
type Selection={index:number;quantity:number;restock:boolean};
...
export default function ReceiptManager({close,allowed,onChanged}:{close:()=>void;allowed:boolean;onChanged:()=>Promise<void>}) {
 ...
 function choose(s:Sale,k:"cancellation"|"return") {setSale(s);setKind(k);setPayment(s.payment);setReason(k==="return"?"Freiwillige Rückgabe":reversalReasons[0]);setNote("");setConfirmed(false);setError("");setLines(s.items.map((l,index)=>({index,quantity:k==="return"?0:remaining(s,index),restock:k==="return"})));}
 ...
 <p>{kind==="return"?`Rücknahme bis ${new Date(returnDeadline(sale)+"T12:00:00Z").toLocaleDateString("de-DE")}: nur unbenutzte, vollständige, freigegebene Nicht-Lebensmittel mit Originalbon. Gesetzliche Mängelrechte bleiben unberührt.`:"Positionen und Mengen auswählen. Nur verkaufsfähige Ware zurück ins Lager buchen; Bruch und mangelhafte Ware bleiben aus dem verfügbaren Bestand entfernt."}</p>
 <fieldset disabled={busy||!!pending} className="reversal-fields">
 {sale.items.map((item,index)=>{const max=remaining(sale,index);const eligible=kind!=="return"||(item.quantity>0&&item.return_eligible&&voluntaryReturnOpen(sale));return <div className="reversal-line" key={index}><div><strong>{item.name}</strong><small>{euro(item.price_cents+item.deposit_cents)} je Einheit · noch {max} korrigierbar</small>{!eligible&&<small>Keine freiwillige Rücknahme</small>}</div><label>Menge<NumberInput aria-label={`Rückgabemenge ${item.name}`} min={0} max={max} value={lines[index]?.quantity||0} disabled={!eligible||!max} onChange={e=>setLines(lines.map(l=>l.index===index?{...l,quantity:Math.min(max,Math.max(0,Number(e.target.value)))}:l))}/></label>{item.quantity>0&&<label className="checkline"><input type="checkbox" checked={lines[index]?.restock||false} disabled={!eligible} onChange={e=>setLines(lines.map(l=>l.index===index?{...l,restock:e.target.checked}:l))}/>Verkaufsfähig zurück ins Lager</label>}</div>})}
 <label>Grund<select value={reason} onChange={e=>setReason(e.target.value)}>{(kind==="return"?["Freiwillige Rückgabe"]:reversalReasons).map(r=><option key={r}>{r}</option>)}</select></label>
 <label>Erläuterung (Pflicht)<textarea value={note} minLength={3} maxLength={1000} onChange={e=>setNote(e.target.value)} placeholder="Was wurde korrigiert und warum?"/></label>
 ...
 <button className="button" disabled={busy||(!pending&&(!confirmed||note.trim().length<3||!lines.some(l=>l.quantity>0)))} onClick={book}>{busy?"Wird gespeichert …":pending?"Denselben Vorgang erneut prüfen":"Gegenbeleg verbindlich buchen"}</button>
