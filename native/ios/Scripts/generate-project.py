...
for name,flag,bundle,device in [('EliasCustomer','CUSTOMER_APP','de.getraenkeelias.kunden','1,2'),('EliasPOS','POS_APP','de.getraenkeelias.kasse','2')]:
    ...
    for is_test in [False,True]:
        ...
        for config in ['Debug','Release']:
            values={'PRODUCT_NAME':'$(TARGET_NAME)','PRODUCT_BUNDLE_IDENTIFIER':bundle+('.uitests' if is_test else ''),'SWIFT_VERSION':'5.0','SWIFT_ACTIVE_COMPILATION_CONDITIONS':flag+(' DEBUG' if config=='Debug' else ''),'IPHONEOS_DEPLOYMENT_TARGET':'17.0','SDKROOT':'iphoneos','TARGETED_DEVICE_FAMILY':device,'CODE_SIGN_STYLE':'Automatic','GENERATE_INFOPLIST_FILE':'YES' if is_test else 'NO','SWIFT_OPTIMIZATION_LEVEL':'-Onone' if config=='Debug' else '-O','ENABLE_TESTABILITY':'YES' if config=='Debug' else 'NO','SUPPORTED_PLATFORMS':'iphoneos iphonesimulator','SUPPORTS_MACCATALYST':'NO','CLANG_ENABLE_MODULES':'YES','SWIFT_EMIT_LOC_STRINGS':'YES','MARKETING_VERSION':'0.1.0','CURRENT_PROJECT_VERSION':'1'}
