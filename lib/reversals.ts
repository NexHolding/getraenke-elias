...
export const reversalSchema = z.object({
 id:z.uuid(),original_sale_id:z.uuid(),kind:z.enum(["cancellation","return"]),reason:z.string().min(3).max(200),note:z.string().trim().min(3,"Bitte den Vorgang kurz erläutern.").max(1000),payment:z.enum(["cash","card"]),confirmed:z.literal(true),
 lines:z.array(z.object({index:z.number().int().min(0).max(210),quantity:z.number().int().min(1).max(1000),restock:z.boolean()})).min(1).max(211),
