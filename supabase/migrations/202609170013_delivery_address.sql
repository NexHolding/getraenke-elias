-- Structured delivery addresses; original address strings and historical documents stay intact.
alter table customers add column street text not null default '',add column house_number text not null default '',add column postal_code text not null default '',add column city text not null default '';
alter table orders add column street text not null default '',add column house_number text not null default '',add column postal_code text not null default '',add column city text not null default '';
-- Existing geocodes must never survive an actual delivery-address change.
create or replace function customer_address_changed() returns trigger language plpgsql as $$begin
 if NEW.address is distinct from OLD.address then NEW.latitude:=null;NEW.longitude:=null;end if;return NEW;
end;$$;
create trigger customer_address_geocode before update on customers for each row execute function customer_address_changed();

-- Recurring orders snapshot the four fields as well.
create or replace function generate_subscription_orders() returns integer language plpgsql security definer set search_path=public as $$declare s subscriptions;c customers;l jsonb;p products;rows jsonb;n integer:=0;next_day date;today date;begin
 ...
 insert into orders(customer_id,customer_name,email,phone,address,street,house_number,postal_code,city,notes,items,status,preference_snapshot,subscription_id,recurrence_date) values(c.id,c.name,c.email,c.phone,c.address,c.street,c.house_number,c.postal_code,c.city,'Wiederkehrende Lieferung',rows,'confirmed',jsonb_build_object('windows',c.windows,'dropoff_allowed',c.dropoff_allowed,'dropoff_note',c.dropoff_note,'latitude',c.latitude,'longitude',c.longitude),s.id,s.next_date) on conflict do nothing;if found then n:=n+1;end if;
